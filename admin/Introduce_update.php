...
<?php
  if(isset($_POST['update'])){
    $ids = $_POST['update'];
    $duction1 = $_POST['intro1'];
    $induction2 = $_POST['intro2'];

    // $target = "research_image/".basename($_FILES['image']['name']);
    // $file = $_FILES['image']['name'];
      $upd = mysqli_query($connect,"UPDATE introduction SET  intro1 = '".$duction1."' , intro2 = '".$induction2."' WHERE id = '".$ids."' ");
      if($upd){
    ...
    <div class="panel-group" data-as-sortable="vm.documentDragListeners" ng-model="vm.documents">
      <div class="panel panel-default xlist" ng-repeat="doc in vm.documents" style="border-bottom: none;" data-as-sortable-item>        
        <div id="collapse{{$index}}" class="panel-collapse collapse in">
          ...
          <button class="btn btn-primary" id="view" onclick="window.location.href='introduce.php'" style="display: block;" ngf-max-height="1000" ngf-max-size="5MB">
                    View introduction</button>
                    <br>
            <form  action="introduce_update.php" method="POST"  enctype="multipart/form-data" >
            <table class="table table-bordered" id="tablepagination" >
             <tbody>
            <?php 

            if($introduc != false){
              foreach($introduc as $r){
                $newid = $r['id'];
            ?>
                <tr>
                    <td class="text-center uname">Introduction1</td>
                    <td>
                      <textarea name="intro1"  class="form-control"  placeholder="필수" aria-describedby="inputGroup-sizing-sm" cols="200" rows="1.8"  ><?= $r['intro1']?></textarea>
                  </td>
                </tr>
                <tr>
                <td class="text-center email">Introduction2</td>
                    <td><textarea name="intro2"  class="form-control" placeholder="필수" aria-describedby="inputGroup-sizing-sm"  cols="100" rows="7"  ><?= $r['intro2']?></textarea></td>
                  </td>
                  </tr>
               <tr>
                <td colspan="2" class="text-center">
                  <div>
                  <button   class="btn btn-primary" id="update" name="update"  value="<?= $r['id']?>" style="width: 120px; border:none; background-color:#F5F5F5; color:black" >Save</button>
